...
class GraphUserActivityWeek(commands.Cog):
    ...
    @commands.command(name="g_user_activity_week")
    async def generate_graph(self, ctx, user: str = None):
        ...
        try:
            ...
            start_date = datetime.utcnow() - timedelta(days=7)  # Starting from 7 days ago
            ...
            all_dates = [(california_start_date + timedelta(days=i)).strftime('%Y-%m-%d') for i in range(8)]  # 7 days + today
...
async def setup(bot):
    await bot.add_cog(GraphUserActivityWeek(bot))